...
SampleCoach::initImpl( CmdLineParser & cmd_parser )
{
    ...
    if ( config().useTeamGraphic() )
    {
        ...
        {
            M_team_graphic.createXpmTiles( team_logo_xpm );
            std::cout << config().teamName() << " coach: team_graphic loaded (embedded), tiles="
                      << M_team_graphic.tiles().size() << std::endl;
        }
        else
        {
            if ( M_team_graphic.readXpmFile( config().teamGraphicFile().c_str() ) )
            {
                std::cout << config().teamName() << " coach: team_graphic loaded from "
                          << config().teamGraphicFile() << ", tiles="
                          << M_team_graphic.tiles().size() << std::endl;
            }
            else
            {
                std::cout << config().teamName() << " coach: WARNING failed to load team_graphic from "
                          << config().teamGraphicFile() << std::endl;
            }
        }
    }
...
SampleCoach::actionImpl()
{
    ...
    // Enviar team graphic nos primeiros ciclos (não só 0: o coach pode conectar depois)
    if ( world().time().cycle() <= 20
         && config().useTeamGraphic()
         && M_team_graphic.tiles().size() != teamGraphicOKSet().size() )
    {
        sendTeamGraphic();
