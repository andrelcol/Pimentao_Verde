...
echo "******************************************************************"
echo " This program is modified by Rinobot (Pimentao_Verde)."
echo " Copyright 2025. Rinobot."
echo " All rights reserved."
echo "******************************************************************"
echo " Based on agent2d / CYRUS / HELIOS (see CREDITS.md)."
echo "******************************************************************"



DIR="$(cd "$(dirname "$0")" && pwd)"
# shellcheck source=../script/setup_libpath.sh
. "${DIR}/../script/setup_libpath.sh" 2>/dev/null \
  || . "${DIR}/../../script/setup_libpath.sh" 2>/dev/null \
  || true
if ! pimentao_export_libpath "${DIR}" 2>/dev/null; then
  echo "Aviso: librcsc não encontrada. Corra ./script/build_all.sh na raiz do projeto." >&2
fi
