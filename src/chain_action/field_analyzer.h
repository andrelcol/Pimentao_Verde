...
public:
    ...
    static bool isOpponentCyrusTeam(const rcsc::WorldModel & wm);
    ...
    /*!
      ...
      O servidor não envia flag «gol vazio»; usa-se posição do jogador com unum de GR.
     */
    static bool isTheirGoalieOutOfGoal( const rcsc::WorldModel & wm );

    /*!
      \brief GR adversário dentro da área penal deles (ServerParam::theirPenaltyArea()).
      Posições globais vêm do WorldModel; não é preciso d/cos manualmente.
      \param error_thr Rect2D::contains(pos, thr): >0 alarga (tolerância), <0 aperta.
     */
    static bool isOpponentGoalieInTheirPenaltyArea( const rcsc::WorldModel & wm,
                                                     const double error_thr = 0.0 );
