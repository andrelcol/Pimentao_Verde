...
# Scripts no pacote: lib fica em ./lib (não depende do caminho da máquina que compilou)
if [ -f "${ROOT}/script/setup_libpath.sh" ]; then
  cp "${ROOT}/script/setup_libpath.sh" "${ROOT}/dist_tmp/${PACKAGE_NAME}/"
fi
for _sh in github.sh auto.sh train.sh keepaway.sh start-debug.sh start-offline.sh; do
  _f="${ROOT}/dist_tmp/${PACKAGE_NAME}/${_sh}"
  [ -f "$_f" ] || continue
  if grep -q 'pimentao_export_libpath\|pimentao_find_libpath' "$_f" 2>/dev/null; then
    continue
  fi
  sed -i 's|LIBPATH=.*pimentao_verde_lib.*|LIBPATH=./lib|' "$_f" 2>/dev/null || true
  sed -i 's|LIBPATH=.*cyruslib.*|LIBPATH=./lib|' "$_f" 2>/dev/null || true
done
